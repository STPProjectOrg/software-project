...
def create_message(request, chat_participant_name):
    ...
    create_or_update_inbox_participant(
        request.user, chat_participant, data["message"])
    create_or_update_inbox_participant(
        chat_participant, request.user, data["message"])

    send_notification(channel_layer, chat_participant.pk, "message",
                      f"{request.user.username} send you a message.")
